...
# Note: BEFORE RUNNING, one must change the file path in line 25

# Note: BEFORE RUNNING, one must change the file path assigned to the variable "excelpath" in line 171

# Note: Batch normalization is implemented but still untested, the effect of dropout layers is unknown


# Read in data, separate into training sets
# (for non-adjacent columns can use np.r_[1,3:5,etc.])
df = pd.read_excel(r'C:\Users\mwh85\Documents\Applied Energy Paper\Analysis While Waiting for Feedback\Haefner_WAVE_Simulations_Trimmed_waiting.xlsx')
x = df.iloc[:,1:4]
y = df.iloc[:,10]
