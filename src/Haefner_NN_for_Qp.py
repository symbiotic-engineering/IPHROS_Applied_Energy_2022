...
df = pd.read_excel('..\data\Haefner_WAVE_Simulations_Trimmed.xlsx')
x = df.iloc[:,1:4]
y = df.iloc[:,10]
...
if np.allclose(my_results.round(decimals=4),predicted_y.round(decimals=4)) == True:
    ...
    # Excel path
    workbook = xlsxwriter.Workbook('RO_weights_trial.xlsx')
    ...
    with pd.ExcelWriter('RO_weights_trial.xlsx') as writer:
        while w <= w_max:
